...
export function projectLinedef(camera: Camera, sector: Sector, linedef: Linedef) : LinedefProjection | null {
  ...
  const startScale = caclulateScaleFactor(startScreenX, sector, linedef, camera);
  const endScale = caclulateScaleFactor(endScreenX, sector, linedef, camera);

  return {
    start: {
      screenX: startScreenX,
      height: sector.height! * startScale / distanceToCamera
    },
    end: {
      screenX: endScreenX,
      height: sector.height! * endScale / distanceToCamera
    }
  };
}
